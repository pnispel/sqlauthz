...
function parseVar(value) {
    ...
    if (parts.length !== 2) {
        throw new Error(`Invalid variable value: ${value}. Must use name=value syntax`);
    }
    const key = parts[0];
...
async function main() {
    ...
    const args = await yargs(hideBin(process.argv))
        ...
        alias: "d",
        type: "string",
        description: "Database URL to connect to. Note that you can " +
        ...
        type: "boolean",
        description: "Revoke existing permissions from any user who matches " +
        ...
        .option("revoke-all", {
        type: "boolean",
        description: "Revoke permissions from all users in the database other " +
        ...
        type: "string",
        array: true,
        description: "Revoke permissions from an explicit list of users before " +
        ...
        type: "boolean",
        description: "Allow rules that do not limit the `actor` in any way. This is " +
        ...
        type: "string",
        array: true,
        description: "Define variable(s) that can be referenced in your rules files " +
        ...
        type: "string",
        array: true,
        description: "File paths to .js scripts or JSON files that will be loaded, " +
            "and the exports will be available in your rules files as " +
            "var.<name>.",
    })
        .option("dry-run", {
        type: "boolean",
        description: "Print full SQL query that would be executed; --dry-run-short only " +
            "includes grants.",
        conflicts: ["dry-run-short"],
    })
        .option("dry-run-short", {
        type: "boolean",
        description: "Print GRANT statements that would be generated without running them.",
        conflicts: ["dry-run"],
    })
        .option("debug", {
        type: "boolean",
    ...
    for (const varFile of varFiles) {
        if (varFile.endsWith(".json")) {
            ...
            console.error(`Invalid var file: ${varFile}. Extension must be .js or .json`);
            process.exit(1);
        }
    }
    try {
    ...
    if (args.databaseUrl.startsWith(envVariablePrefix)) {
        ...
        if (!envVariable) {
            console.error(`Invalid environment variable specified for databaseUrl: ${envVariableName}`);
            process.exit(1);
        }
